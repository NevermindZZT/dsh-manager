(() => {
  ...
  async function refreshPairing() { try { const info = await api("/api/v1/admin/pairing/refresh", { method: "POST" }); $("pairingCode").textContent = info.pairingCode || ""; } catch (error) { $("error").textContent = error.message; } }
